...
문제(페인포인트)에서 출발해 아이디어를 발굴하고 `~/ideas/`에 축적·관리하는 Claude Code 스킬. 도메인별 방법론, 증거 등급, 독립 비평을 갖춘 v0.2.0 기준이다.
...
| 2. 해법 발산 | 문제당 5~8개, 서로 다른 발산 기법으로 강제 분산. 기존 해결책 사전 조사 | — |
...
- **증거 등급**: 모든 근거에 E1(1차 증거·원문 인용) / E2(2차 증거·기사·리포트) / E3(추론) 등급을 표기한다. "왜 아픈가"의 근거가 E3뿐이면 신뢰도 낮음으로 표시한다.
- **기존 해결책 사전 조사**: 해법을 내기 전에 이미 존재하는 제품·OSS를 찾는다. 존재하면 "왜 그것으로 충분하지 않은가"에 답해야 통과한다 — 뻔한 재발명을 거르는 가장 강한 필터다.
- **프리모템**: "6개월 뒤 실패했다"고 단정하고 원인을 역산한다.
- **독립 레드팀** (깊은 모드): 서브에이전트에게 아이디어 요약과 문제 정의만 넘기고(생성 맥락은 차단) "옹호하지 말고 죽여라"라고 지시한다.
...
├── 2026-08-05-<slug>.md            # 개별 아이디어 (기존 대안, 프리모템, 실험 로그 포함)
└── sessions/2026-08-05-<topic>.md  # 탈락한 해법까지 포함한 세션 원본
```

- 이 폴더는 저장소 밖, 홈 디렉토리에 만들어진다. 플러그인을 설치한다고 자동으로 생기지 않고 스킬을 처음 실행할 때 생성된다.
- 상태값은 `inbox`(발굴 직후) → `exploring`(검증 중) → `parked`(보류, 재검토 날짜 필수) / `building`(실행 중) / `killed`(폐기, 사유 필수) 5가지다.
...
└── plugins/idea-mining/
    ...
    └── skills/idea-mining/
        ...
        └── references/
            ...
            ├── evidence.md             # 증거 등급, 기존 해결책 사전 조사
            ├── divergence.md           # 발산 기법 10종
            ├── critique.md             # 프리모템, 독립 레드팀, 강제 순위
