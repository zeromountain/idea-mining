...
Critique (`references/critique.md` §3) is a loop, not a single pass: after a rebuttal, the idea gets pivoted and re-critiqued until the red team/premortem rates its survival confidence "상" (high) or a 3-round cap is hit — this exists because an earlier one-shot version was killing ideas on their first rebuttal, before they'd had a chance to be refined. Rebuttals are also tagged "구조적" (structural — no assumption change saves it, kills immediately) vs "완화 가능" (fixable — drives the next pivot); only a structural rebuttal short-circuits the loop early. If you touch this flow, keep both the round cap (uncapped retries defeat the point of a critique gate) and the structural/fixable split (removing it is what caused the original over-killing complaint).
